...
if(empty($_GET['r'])) {
	...
	$releasequery = $oar_sql->query('SELECT * FROM ' . $sql->get_table_prefix() . 'releases WHERE releaseid = "' . $releaseid . '"');
	if($oar_sql->error()) {
...
<html prefix="og: http://ogp.me/ns# music: http://ogp.me/ns/music#">
	<head>
		<meta charset="UTF-8"/>
		<title>"<?php echo $releasename;?>" by <?php echo $oar_config['artist']; ?></title>
